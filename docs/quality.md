...
|---|---|---|
| Swift build | The app target builds from a clean checkout. | `xcodebuild -project BeanBook.xcodeproj -scheme BeanBook -destination 'platform=iOS Simulator,name=iPhone 16 Pro,OS=26.0' build` locally; generic simulator build in CI. |
| Swift tests | Unit tests use Swift Testing; UI tests use XCTest only. | `xcodebuild test -project BeanBook.xcodeproj -scheme BeanBook -destination 'platform=iOS Simulator,name=iPhone 16 Pro,OS=26.0'` |
| Functions build | TypeScript compiles without generated output drift. | `cd functions && npm run build` |
...
1. **Docs-only:** `./scripts/agent-preflight.sh`
2. **Swift compile:** local `xcodebuild ... build`
3. **Swift unit tests:** local `xcodebuild ... test`
4. **Functions:** `cd functions && npm run build`
...
- CI currently verifies build only; local test runs remain the stronger signal.
